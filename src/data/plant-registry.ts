import type { PlantDefinition } from "@/types/plant";
import { PlantCategory, PlantType } from "@/types/plant";

export const PLANT_REGISTRY: Record<PlantType, PlantDefinition> = {
  [PlantType.RainReed]: {
    id: PlantType.RainReed,
    name: "Rain Reed",
    label: "Rain",
    description: "Gentle rain-like white noise for calm and stability",
    color: "#7BA7BC",
    svgPath: "/plants/rain-reed.svg",
    audioPath: "/audio/rain-reed.ogg",
    category: PlantCategory.Ambient,
  },
  [PlantType.LofiFern]: {
    id: PlantType.LofiFern,
    name: "Lofi Fern",
    label: "Piano",
    description: "Dreamy lo-fi electric piano chord progressions",
    color: "#6B8E23",
    svgPath: "/plants/lofi-fern.svg",
    audioPath: "/audio/lofi-fern.ogg",
    category: PlantCategory.Melodic,
  },
  [PlantType.PulseMoss]: {
    id: PlantType.PulseMoss,
    name: "Pulse Moss",
    label: "Bass",
    description: "Deep heartbeat-like synthesizer bass pulses",
    color: "#4A7C59",
    svgPath: "/plants/pulse-moss.svg",
    audioPath: "/audio/pulse-moss.ogg",
    category: PlantCategory.Rhythmic,
  },
  [PlantType.BellFlower]: {
    id: PlantType.BellFlower,
    name: "Bell Flower",
    label: "Bell",
    description: "Crystal-clear high-pitched bell melodies",
    color: "#C9A0DC",
    svgPath: "/plants/bell-flower.svg",
    audioPath: "/audio/bell-flower.ogg",
    category: PlantCategory.Melodic,
  },
  [PlantType.WindWood]: {
    id: PlantType.WindWood,
    name: "Wind Wood",
    label: "Wood",
    description: "Organic wood percussion clicks and knocks",
    color: "#8B6914",
    svgPath: "/plants/wind-wood.svg",
    audioPath: "/audio/wind-wood.ogg",
    category: PlantCategory.Rhythmic,
  },
  [PlantType.HazeLily]: {
    id: PlantType.HazeLily,
    name: "Haze Lily",
    label: "Haze",
    description: "Warm brown noise with vintage texture",
    color: "#C4A882",
    svgPath: "/plants/haze-lily.svg",
    audioPath: "/audio/haze-lily.ogg",
    category: PlantCategory.Ambient,
  },
  [PlantType.RustleIvy]: {
    id: PlantType.RustleIvy,
    name: "Rustle Ivy",
    label: "Rustle",
    description: "Rustling pink noise like wind through leaves",
    color: "#7A9E7E",
    svgPath: "/plants/rustle-ivy.svg",
    audioPath: "/audio/rustle-ivy.ogg",
    category: PlantCategory.Ambient,
  },
  [PlantType.TideSeaweed]: {
    id: PlantType.TideSeaweed,
    name: "Tide Seaweed",
    label: "Tide",
    description: "Slow wave sweep like ocean tides",
    color: "#6B8F9E",
    svgPath: "/plants/tide-seaweed.svg",
    audioPath: "/audio/tide-seaweed.ogg",
    category: PlantCategory.Ambient,
  },
  [PlantType.ShimmerSage]: {
    id: PlantType.ShimmerSage,
    name: "Shimmer Sage",
    label: "Shimmer",
    description: "Tremolo pad with gentle shimmer",
    color: "#A3B18A",
    svgPath: "/plants/shimmer-sage.svg",
    audioPath: "/audio/shimmer-sage.ogg",
    category: PlantCategory.Ambient,
  },
  [PlantType.EchoVine]: {
    id: PlantType.EchoVine,
    name: "Echo Vine",
    label: "Echo",
    description: "Delay feedback pad with echoing trails",
    color: "#5B9279",
    svgPath: "/plants/echo-vine.svg",
    audioPath: "/audio/echo-vine.ogg",
    category: PlantCategory.Pads,
  },
  [PlantType.DriftWillow]: {
    id: PlantType.DriftWillow,
    name: "Drift Willow",
    label: "Drift",
    description: "Lush string pad drifting through harmonies",
    color: "#8E9AAF",
    svgPath: "/plants/drift-willow.svg",
    audioPath: "/audio/drift-willow.ogg",
    category: PlantCategory.Pads,
  },
  [PlantType.HumLotus]: {
    id: PlantType.HumLotus,
    name: "Hum Lotus",
    label: "Hum",
    description: "Deep choral hum with reverb wash",
    color: "#D4A0A0",
    svgPath: "/plants/hum-lotus.svg",
    audioPath: "/audio/hum-lotus.ogg",
    category: PlantCategory.Pads,
  },
  [PlantType.EmberThorn]: {
    id: PlantType.EmberThorn,
    name: "Ember Thorn",
    label: "Ember",
    description: "Warm distorted drone with glowing character",
    color: "#C45B28",
    svgPath: "/plants/ember-thorn.svg",
    audioPath: "/audio/ember-thorn.ogg",
    category: PlantCategory.Pads,
  },
  [PlantType.CrystalCactus]: {
    id: PlantType.CrystalCactus,
    name: "Crystal Cactus",
    label: "Crystal",
    description: "FM bell arpeggio with crystalline tones",
    color: "#70C1B3",
    svgPath: "/plants/crystal-cactus.svg",
    audioPath: "/audio/crystal-cactus.ogg",
    category: PlantCategory.Melodic,
  },
  [PlantType.ChirpClover]: {
    id: PlantType.ChirpClover,
    name: "Chirp Clover",
    label: "Chirp",
    description: "Music box pluck melody like chirping",
    color: "#95B46A",
    svgPath: "/plants/chirp-clover.svg",
    audioPath: "/audio/chirp-clover.ogg",
    category: PlantCategory.Melodic,
  },
  [PlantType.TwangBamboo]: {
    id: PlantType.TwangBamboo,
    name: "Twang Bamboo",
    label: "Twang",
    description: "Pentatonic pluck with bamboo-like resonance",
    color: "#C7956D",
    svgPath: "/plants/twang-bamboo.svg",
    audioPath: "/audio/twang-bamboo.ogg",
    category: PlantCategory.Melodic,
  },
  [PlantType.FrostOrchid]: {
    id: PlantType.FrostOrchid,
    name: "Frost Orchid",
    label: "Frost",
    description: "Crystal high tones with icy delay trails",
    color: "#7BBCD4",
    svgPath: "/plants/frost-orchid.svg",
    audioPath: "/audio/frost-orchid.ogg",
    category: PlantCategory.Melodic,
  },
  [PlantType.SparkDaisy]: {
    id: PlantType.SparkDaisy,
    name: "Spark Daisy",
    label: "Spark",
    description: "Sparse metallic chime hits like sparks",
    color: "#E8B960",
    svgPath: "/plants/spark-daisy.svg",
    audioPath: "/audio/spark-daisy.ogg",
    category: PlantCategory.Rhythmic,
  },
  [PlantType.GrooveRoot]: {
    id: PlantType.GrooveRoot,
    name: "Groove Root",
    label: "Groove",
    description: "Lo-fi kick drum pulse from the roots",
    color: "#8B6C5C",
    svgPath: "/plants/groove-root.svg",
    audioPath: "/audio/groove-root.ogg",
    category: PlantCategory.Rhythmic,
  },
  [PlantType.BubbleKelp]: {
    id: PlantType.BubbleKelp,
    name: "Bubble Kelp",
    label: "Bubble",
    description: "Random water bubble pops and drips",
    color: "#4CA1A3",
    svgPath: "/plants/bubble-kelp.svg",
    audioPath: "/audio/bubble-kelp.ogg",
    category: PlantCategory.Rhythmic,
  },
  [PlantType.SmokyJasmine]: {
    id: PlantType.SmokyJasmine,
    name: "Smoky Jasmine",
    label: "Keys",
    description: "Jazzy Rhodes electric piano comping with extended voicings",
    color: "#C4A265",
    svgPath: "/plants/smoky-jasmine.svg",
    audioPath: "/audio/smoky-jasmine.ogg",
    category: PlantCategory.Melodic,
  },
  [PlantType.StrollMangrove]: {
    id: PlantType.StrollMangrove,
    name: "Stroll Nopal",
    label: "Walk",
    description: "Walking bass line with chromatic approach notes",
    color: "#9C7068",
    svgPath: "/plants/stroll-mangrove.svg",
    audioPath: "/audio/stroll-mangrove.ogg",
    category: PlantCategory.Rhythmic,
  },
  [PlantType.BrushThistle]: {
    id: PlantType.BrushThistle,
    name: "Brush Thistle",
    label: "Brush",
    description: "Brush drum swing with ride cymbal and ghost snare",
    color: "#8B7D6B",
    svgPath: "/plants/brush-thistle.svg",
    audioPath: "/audio/brush-thistle.ogg",
    category: PlantCategory.Rhythmic,
  },
  [PlantType.CroonMagnolia]: {
    id: PlantType.CroonMagnolia,
    name: "Croon Magnolia",
    label: "Horn",
    description: "Muted jazz horn melody with blue notes and vibrato",
    color: "#A882A0",
    svgPath: "/plants/croon-magnolia.svg",
    audioPath: "/audio/croon-magnolia.ogg",
    category: PlantCategory.Melodic,
  },
};

export const PLANT_LIST = Object.values(PLANT_REGISTRY);

export const PLANT_CATEGORIES: {
  category: PlantCategory;
  label: string;
  plants: PlantDefinition[];
}[] = [
  {
    category: PlantCategory.Ambient,
    label: "Ambient",
    plants: PLANT_LIST.filter((p) => p.category === PlantCategory.Ambient),
  },
  {
    category: PlantCategory.Melodic,
    label: "Melodic",
    plants: PLANT_LIST.filter((p) => p.category === PlantCategory.Melodic),
  },
  {
    category: PlantCategory.Rhythmic,
    label: "Rhythmic",
    plants: PLANT_LIST.filter((p) => p.category === PlantCategory.Rhythmic),
  },
  {
    category: PlantCategory.Pads,
    label: "Pads",
    plants: PLANT_LIST.filter((p) => p.category === PlantCategory.Pads),
  },
];
